...
//				$this->strLabel = $params['name'];
	...
	protected function isProjectDB()
	{
		...
		if("Fornecedores" == $this->chrt_array['tables'][0])
			return true;
		if("Categorias" == $this->chrt_array['tables'][0])
			return true;
		if("Materiais" == $this->chrt_array['tables'][0])
			return true;
		if("Ponto de Entrada" == $this->chrt_array['tables'][0])
			return true;
		if("Armazenagem" == $this->chrt_array['tables'][0])
			return true;
		if("Projeto de Desenvolvimento de Sistemas_Usuarios" == $this->chrt_array['tables'][0])
			return true;
		if("Projeto de Desenvolvimento de Sistemasugrights" == $this->chrt_array['tables'][0])
			return true;
		return false;
	}
...
//		$strDataSeries = $row[ $this->arrDataSeries[ $seriesNumber ] ];
		...
		return array( "x" => $strLabelFormat , "value" => $this->chart_xmlencode( (double)str_replace(",", ".", $strDataSeries) ) );
	}
...
class Chart_Bubble extends Chart
{
	...
	protected function getPoint( $seriesNumber, $row )
	{
		...
		$pointData["size"] = $this->chart_xmlencode( (double)str_replace(",", ".", $row[ $this->arrDataSize[ $seriesNumber ] ])  );
...
class Chart_Ohlc extends Chart
{
	...
	protected function getPoint( $seriesNumber, $row )
	{
		...
		return array(
			...
			"open" => $this->chart_xmlencode( (double)$open ),
			"high" => $this->chart_xmlencode( (double)$high ),
			"low" => $this->chart_xmlencode( (double)$low ),
			"close" => $this->chart_xmlencode( (double)str_replace(",", ".", $close) )
		);
	}
}
?>