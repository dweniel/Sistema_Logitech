...
class LoginPage extends RunnerPage
{
	...
	protected function prepareTwoFactorMessage() {
		...
		if( $destination["method"] === "phone" ) {
			$twofMessage = str_replace( "%phone%", $destination["address"], "Uma mensagem de texto com o seu código foi enviada para: %phone%" );
		} else if( $destination["method"] === "email" ) {
			$twofMessage = str_replace( "%email%", $destination["address"], "Um e-mail com seu código foi enviada para %email%." );
		} else if( $destination["method"] === "totp" ) {
			$twofMessage = str_replace(
				array( "%username%", "%site%" ),
				array( "<br><b>".Security::provisionalUsername()."</b>", "<b>".$destination["address"]."</b>" ),
				"Digite o código de seu aplicativo de autenticação correspondente a %username% no %site%."
			);
			$this->hideItemType("resend_button");
		}
	...
	protected function sendTwoFactorCode()
	{
		...
		if( !$ret["success"] )
		{
			$this->message = "envio de mensagem de erro"." ".$ret["error"];
	...
	protected function refineMessage()
	{
		if( $this->message == "expired" )
			$this->message = "Sua sessão expirou." . " " . "Favor logar novamente.";
	...
	protected function isLoginAccessAllowed()
	{
		...
		$this->message = mysprintf( "Acesso negado po %s minutos", array($this->auditObj->LoginAccess()) );
	...
	protected function createDBSesssion( $provider, $username, $password ) {
		...
		if( Security::verifyTwoFactorEnabled( $data ) ) {
			if( !Security::machineTwoFactorTrusted( $data ) ) {
				Security::createProvisionalSession( $provider, LOGGED_2F_PENDING, $username, $displayName, $data );
				return true;
			}
		} else {
			...
			if( $twofSettings["required"] ) {
				Security::createProvisionalSession( $provider, LOGGED_2FSETUP_PENDING, $username, $displayName, $data );
				return true;
			}
		}
	...
	protected function createADSesssion( $provider, $username, $password ) {
		...
		return $plugin->createUserSession( $userInfo );
	}
	...
	protected function assignBody()
	{
		...
		$this->body["end"] .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/runnerJS/RunnerAll.js?38328")."\"></script>";
