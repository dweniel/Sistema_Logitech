...
class RegisterPage extends RunnerPage
{
	...
	protected function checkRegisterData( $strUsername, $strPassword, $strEmail )
	{
		...
		{
			$this->jsSettings['tableSettings'][ $this->tName ]['msg_userError'] = "Nome do usuário não pode estar vazio";
			$ret = false;
		}
		else if( !$this->checkIfUsernameUnique( $strUsername ) )
		{
			$this->jsSettings['tableSettings'][ $this->tName ]['msg_userError'] = "Nome do Usuário"." <i>".runner_htmlspecialchars( $strUsername )."</i> "."já existe. Escolha outro nome";
			$ret = false;
		}
		...
		if( Security::emailField() && $this->pSet->appearOnPage( Security::emailField() ) )
		{
			...
			if( !strlen($strEmail) )
			{
				$this->jsSettings['tableSettings'][ $this->tName ]['msg_emailError'] = "Favor entrar com email válido";
				$ret = false;
			}
			else if( !$this->checkIfEmailUnique( $strEmail ) )
			{
				$this->jsSettings['tableSettings'][ $this->tName ]['msg_emailError'] = "Email"." <i>". runner_htmlspecialchars( $strEmail )."</i> "."já registrado. Se você esqueceu seu Nome de Usuário ou Senha, use o formulário de ‘Esqueci Minha Senha’";
				$ret = false;
			}
		}
