...
class RightsPage extends ListPage
{
	...
	function fillGroupsArr()
	{
		...
		$this->groups[-2] = array( "label" => "<"."Padrão".">" );
		$this->groups[-3] = array( "label" => "<"."Visitante".">" );
	...
	function getRights()
	{
		...
		$sql = "select ". $this->connection->addFieldWrappers( "GroupID" )
			...
			." from ". $this->connection->addTableWrappers( "Projeto de Desenvolvimento de Sistemasugrights" )
	...
	function sortTables()
	{
		...
		if(count($this->alphaOrderedTables) > count($addedTables))
		{
			...
			$arr = array();
			$arr["collapsed"] = true;
			$arr["title"] = "Tabelas não listadas";
			$arr["items"] = array();
			$this->menuOrderedTables[] = $arr;
	...
	function updateTablePermissions( $table, $group, $tableRights )
	{
		...
		$rightWTableName = $this->connection->addTableWrappers( "Projeto de Desenvolvimento de Sistemasugrights" );
