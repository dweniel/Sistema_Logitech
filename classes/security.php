...
class Security
{
	...
	{
		echo printJSON(array("success" => false, "message" => "Você não tem permissão para acessar esta tabela" . " " .$message ) );
		exit();
	}
	...
	public static function getUserGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "Label" )
			." from ". $grConnection->addTableWrappers( "Projeto de Desenvolvimento de Sistemasuggroups" ) . " WHERE " . $grConnection->addFieldWrappers( "GroupID" )
	...
	*  For AD it is <provider code><username>
	...
	public static function loginAs( $username, $fireEvents = true, $displayName = "", $userData = array() )
	{
		...
		if( Security::hardcodedLogin() ) {
			...
			//	todo - read usergroups from AD if needed
			Security::createUserSession( $provider, $username, $displayName != "" ? $displayName : $username );
		}
		if( $fireEvents ) {
	...
	public static function getUgMembersDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "Projeto de Desenvolvimento de Sistemasugmembers", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgGroupsDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "Projeto de Desenvolvimento de Sistemasuggroups", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgRightsDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "Projeto de Desenvolvimento de Sistemasugrights", $cman->getUserGroupsConnId() );
	}
