...
function checkTableName($shortTName )
{
	...
	if ("fornecedores" == $shortTName )
		return true;
	if ("categorias" == $shortTName )
		return true;
	if ("materiais" == $shortTName )
		return true;
	if ("ponto_de_entrada" == $shortTName )
		return true;
	if ("armazenagem" == $shortTName )
		return true;
	if ("projeto_de_desenvolvimento_de_sistemas_usuarios" == $shortTName )
		return true;
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Fornecedores");
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Materiais");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Ponto de Entrada");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="Ponto de Entrada";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Armazenagem");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Projeto de Desenvolvimento de Sistemas_Usuarios");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="Projeto de Desenvolvimento de Sistemas_Usuarios";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
...
//	default permissions
	...
	if( $table=="Projeto de Desenvolvimento de Sistemas_Usuarios" )
	{
		if( $sUserGroup=="Funcionarios" )
		{
						return "ADESPI".$extraPerm;
...
//		$permission = Security::pageType2permission( $pSet->getPageType() );
	...
	$restrictedPages = Security::getRestrictedPages( $table );
	return Security::userCanSeePage( $table, $page ) && !$restrictedPages[ $page ];
}
...
function GetBaseScriptsForPage($isDisplayLoading, $additionalScripts = "", $customText = "")
{
	...
	$result = "";
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/loadfirst.js?38328")."\"></script>";


	$result .= $additionalScripts;
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/lang/".getLangFileName(mlang_getcurrentlang()).".js?38328")."\"></script>";
